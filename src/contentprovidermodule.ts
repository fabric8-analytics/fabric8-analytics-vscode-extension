...
export module contentprovidermodule {
  ...
  let render_stack_iframe = (sa, tokenuri) => {
    const result = sa.result[0];
    return `<iframe width="100%" height="100%" frameborder="0" src=' http://fabric8-analytics-stack-report-ui-bayesian-preview.b6ff.rh-idev.openshiftapps.com/#/analyze/${sa.request_id}?interframe=true&api_data={"access_token":"${tokenuri}"}' id="frame2" name="frame2"></iframe>`;
  }
