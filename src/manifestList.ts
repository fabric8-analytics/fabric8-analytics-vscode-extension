...
export class FileSystemProvider implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider {
	...
	async getChildren(element?: Entry): Promise<Entry[]> {
		if (element) {
			...
			children = children.filter((child)=>child[0]=='package.json')
	...
	getTreeItem(element: Entry): vscode.TreeItem {
		...
		if (element.type === vscode.FileType.File) {
			treeItem.command = { command: 'extension.fabric8AnalyticsWidgetFullStack', title: "Run Full Stack Analysis", arguments: [element.uri], };
			treeItem.contextValue = 'file';
			treeItem.description = 'Click to view report';
		}
		return treeItem;
	}
}
